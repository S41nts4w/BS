...
/**
 *****************************************************************************************
 *  @brief      This function updates the page table for page vmem->adm.req_pageno.
...
void vmem_init(void) {
	...
	vmem->adm.size = SHMSIZE; //VMEM_VIRTMEMSIZE
	...
	local_sem = sem_open(NAMED_SEM, O_CREAT, 0644, 3);
...
int find_remove_fifo(void) {
	int frame = removeCounter;
	removeCounter++;
	if(removeCounter>=VMEM_NFRAMES){
		removeCounter = 0;
	}
	return frame;
}
...
void cleanup(void) {
	//TODO Clear of all the things!!! Structs destroy blabla
